...
export const secondaryFunctions = {
    ...
    fastest_speed_cheat() {
      ...
      `);
            ...
            alert(`ТЕБЕ ЗАРІКРОЛИЛИ! В Тебе тепер ще менше оборотів - ${enemyCars.array[enemyCars.index].maxRpm}`);
            ...
            let cheatEffect = setInterval(() => {
                ...
                if (cheatEffectTime == 10) {
                    ...
                    alert("Все, ефект від читів пройшов і в тебе знову 10000 оборотів. Якщо хочеш, можеш знову написати цю команду і твоя машина нарешті отримає масимальну потужність.");
                    if ($("#choose-info").val() == "speedCheat") {
                        $(".explanation-content").html(`Нажми на <button class="cheat-button">Мене</button>, і все станеться.`);
                    }
    ...
    `);
      ...
      `);
        ...
        $(".gears-block").prepend(arrow + "id='gearUpButton' class='playing-btn'/>");
        $(".gears-block").append(arrow + 'style="rotate:180deg" id="gearDownButton" class="playing-btn"/>');
        $(".counters").after('<button id="useTheEngine" class=" playing-btn">ДВИГУН</button>');
        ...
        $("#useTheEngine")[0].addEventListener(functions[0], () => {
    ...
    useLocalStorageAndCookies() {
        ...
        function checkMusic() {
            if (!navigator.cookieEnabled) {
                alert("У вас відключено файли cookie. Тут вони використовуються для того, щоб запам'ятати те - хочете ви слухати музику, чи ні. Для кращих емоцій вам варто дозволити використання цих файлів");
            }
      ...
      `);
                let cheatEffect = setInterval(() => {
                    ...
                    if (cheatEffectTime == 10) {
                        ...
                        alert("Все, ефект від читів пройшов і в тебе знову 10000 оборотів. Якщо хочеш, можеш знову написати цю команду і твоя машина нарешті отримає масимальну потужність.");
                        $(".explanation-content").html(`нажми <button class="cheat-button">Мене</button>, і все станеться`);
                        myCar.maxRpm = 10000;
                        permissions.toCheat = true;
                        clearInterval(cheatEffect);
                        $.removeCookie("cheatEffect", { path: "/" });
            ...
            if (localStorage.getItem("mini-car-racing-progress") &&
                ...
                switch (variables.totalProgress) {
                    ...
                    case "finalRace":
                        ...
                        $(".gameplay-headline").text("Як швидко минає час... Нижче можна скинути прогрес.");
                        break;
                }
                if (variables.totalProgress == "Everything") {
                    ...
                    $(".uncompleted-parts").append(`<button>Скинути прогрес і перезавантажитись?</button>`);
                }
                else {
    ...
    selectInfoInMenu() {
        ...
        switch (topic) {
            case "acceleration":
                $(".explanation-content").html("Для Ефективного набирання швидкості треба розганяти обороти до всіх 10000RPM та переключати передачі вгору в самий останній момент");
                break;
            case "turn":
                $(".explanation-content").html(`Ти повинен бути готовим, що справа на дорозі буде написано: Через "дистанція до поворота" m, "допустима швидкість" km/h. Як тільки це з'явилось - тормози до допустимої швидкості І НЕ РОЗГАНЯЙСЯ, доки не минеш поворот`);
                break;
            case "speedCheat":
                if (!permissions.toCheat) {
                    $(".explanation-content").html(`
      ...
      <span class='undiscovered-music-left'>Назви решти творів ти отримаєш при проходженні гри</span>
        `);
                ...
                break;
            case "device":
                $(".explanation-content").html('Якщо хочеш змінити дезайн і управління на інший девайс, то нажми на <button class="change-device">МЕНЕ</button>');
                break;
        }
    },
    chooseChapter() {
    ...
    createDeviceChangingPopup(permission, changeSelectValue = false, restart = false) {
      ...
      <div class="screen-width">
        ...
        if (changeSelectValue) {
            $("#choose-device").val(variables.device);
        }
        ...
        $(".device-changing-popup button").on("click", function () {
            if ($("#choose-device").val()) {
                let confirmation = confirm("Ви впевнені? Управління потім можна буде змінити в меню паузи");
                if (confirmation) {
