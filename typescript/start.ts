...
function createDeviceChangingPopup(
  ...
  if (localStorage.getItem("device"))
    (<HTMLInputElement>document.getElementById("choose-device")).value =
      localStorage.getItem("device");
    ...
    if (deviceSelect.value != "undefined" && langSelect.value != "undefined") {
      ...
      if (confirmation) {
        ...
        if (!device && deviceSelect.value != "computer") {
          setStylesForPhone(
            langSelect.value as "english" | "ukrainian",
            deviceSelect.value as "mouse" | "phone"
          );
        }
        let arr = variables.currentWindows.split("-");
        arr.pop();
        variables.currentWindows = arr.join("-");
        variables.device = deviceSelect.value as Device;
        variables.language = langSelect.value as "ukrainian" | "english";
        alert(
        ...
        if (variables.language == "english") setLanguageForHtml();
        else
  ...
  if (permission) {
    if (
      ...
      switch (variables.totalProgress) {
        ...
        case "secondRace":
          (
            document.querySelector(".secondRace-begin") as HTMLElement
          ).style.color = "white";
          document
      ...
      switch (variables.totalProgress) {
        case "Everything":
        case "finalRace":
          (document.querySelector(".finalRace-begin") as HTMLElement).style[
            "color"
          ] = "white";
          document
            .querySelector(".completed-parts .parts")
      ...
      (document.querySelector(".gameplay-pause") as HTMLElement).style[
        "opacity"
      ] = "1";
      (document.querySelector(".gameplay-pause") as HTMLElement).style.display =
        "flex";

      document.body.style.overflowY = "scroll";
      (
        document.querySelector(".save-the-progress-button") as HTMLElement
      ).style.cssText = `
        background: white;
        color: black;
...
function setStylesForPhone(
  ...
  (keyboard.children[0] as HTMLElement).onclick = function () {
  ...
  let functions: Array<string> =
    device == "phone" ? ["touchstart", "touchend"] : ["mousedown", "mouseup"];
