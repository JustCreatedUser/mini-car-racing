...
const turns = {
    randomize() {
        ...
        for (let i = 0; i < turnAmount; i++) {
            ...
            let minMaxSpeed = 130 - 15 * difficulty + 40 * turn.type, minDistanceToIt = 300 - 90 * difficulty, minDistanceOfTurning = 300 - 25 * difficulty - 50 * (2 - turn.type), minDistanceAfterTurning = 500 - 100 * difficulty;
    ...
    start() {
        let { maxSpeed, distanceOfTurning, type, direction } = turns.array[turns.index];
    ...
    announce() {
        if (turns.array.length != 0 &&
            ...
            secondaryFunctions.announceFn(turns.array[turns.index].type.toUpperCase() +
    ...
    manage() {
        if (turns.array.length != 0 &&
        ...
        else if (turns.endPosition &&
            turns.isRightNow === true &&
            variables.backgroundPositionX <= turns.endPosition) {
            turns.end();
            turns.endPosition = undefined;
            if (turns.index == turns.array.length) {
                chapters[variables.progress].comingToFinish();
            }
        }
            ...
            if ((variables.progress == "finalRace" &&
...
export { turns };
//# sourceMappingURL=turningFunctions.js.map