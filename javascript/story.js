...
class Race extends Story {
    ...
    beginning(someFn, append, param) {
        ...
        setTimeout(() => {
            ...
            $("footer").css("display", variables.device == "computer" ? "flex" : "grid");
            setTimeout(() => {
                variables.car.style.transition = "unset";
        ...
        secondaryFunctions.announceFn(variables.language == "english" ? "Фінішна пряма" : "The finish line!", () => {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    ...
                    let entry = false, count = 3;
                    intervals.finishing = setInterval(() => {
                        if (!variables.isGamePaused) {
                            switch (count) {
                                case 0:
                                    variables.$announcement.remove();
                                    if (enemyCars.array[enemyCars.index].position >
                                        Number($(myCar.className)
                                            .css("margin-left")
        ...
        setTimeout(() => {
            switch (this) {
                ...
                case finalRace:
                    ...
                    setTimeout(() => {
                        location.replace("https://mini-car-racing.netlify.app/credits.html");
                    }, 2000);
                    break;
            }
            switch (this) {
                ...
                case firstRace:
                    ...
                    setTimeout(() => {
                        ...
                        if (variables.totalProgress != "Everything" &&
                            chapters &&
                            chapters[variables.totalProgress]) {
                            permissions.toSaveProgress = true;
    ...
    startTurning() {
        ...
        changes.movingPause = false;
        variables.race.style.opacity = "0.7";
        let distance = turns.array[turns.index].distanceToIt * variables.distanceRatio;
        let position = variables.backgroundPositionX + distance;
        music.changeVolume(1);
        intervals.turnComing = setInterval(() => {
            if (turns.array.length != 0) {
                if (variables.backgroundPositionX < position &&
                ...
                if (changes.firstRace.continueFirstTurnExplanation)
                    $(".turn-distance").text(Math.round((position - variables.backgroundPositionX) /
                        variables.distanceRatio) + "m");
            }
        }, 100);
    }
}
introduction = new Intro({
    engine() {
        if (variables.action > 0) {
            ...
            setTimeout(() => {
                introduction.tip(variables.language == "ukrainian"
                    ...
                    : `Then increase the engine speed (RPM) at least to 6200. To do this hold ` +
                        ...
                        (variables.device == "computer" ? `left.` : "center."), "#accelerationPedal");
                permissions.forMoreRpm = true;
    ...
    inLessRpm() {
        ...
        if (myCar.rpm < 3000 && !changes.introduction.gearDownAction) {
            myCar.noClutchMode = false;
            introduction.tip(variables.language != "english"
                ? `Останні штрихи: переключи передачу вниз ${variables.device == "computer"
                    ? `клавішою "${keyboard.gearDown}"`
                    : " цією стрілкою"}. Увага: передачу вниз переключай, КОЛИ ОБОРОТИ МЕНШІ ЗА 6000`
                : `Finally, shift gear down ${variables.device == "computer"
                    ? `with "${keyboard.gearDown}"`
                    : "with an Arrow down"}`, "#gearDownButton");
            changes.introduction.gearDownAction = true;
            changes.movingPause = true;
            myCar.noClutchMode = false;
            music.changeVolume(0.5);
    ...
    reachTheTarget() {
        if (variables.action === 3 &&
            ...
            changes.movingPause = true;
            introduction.tip(variables.language != "english"
                ? `Ти майже приїхав, пора тормозити! Зажми ${variables.device == "computer"
    ...
    deccelerationExplanation() {
        if (variables.action == 5 && myCar.gear == 1) {
            music.changeVolume(0.5);
            introduction.tip(variables.language != "english"
                ? `Зараз, коли передача є першою, а тобі треба зупинитись, притормози до меншої за 20 км/год швидкості ${variables.device == "computer"
    ...
    gearDownExplanation() {
        ...
        if (changes.movingPause) {
            ...
            variables.guideBlockText.innerText =
                variables.language != "english"
                    ? `Як ти вже побачив - коробка передач - ${variables.device == "computer"
